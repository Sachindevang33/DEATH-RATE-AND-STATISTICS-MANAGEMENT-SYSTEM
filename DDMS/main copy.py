...
@app.route('/disease')
def disease():
    query=db.engine.execute(f"SELECT * FROM `mrate`") 
...
def addmortalrate():
    ...
    if request.method=="POST":
        ...
        print(morrate,dname)
        atte=Mrate(dname=dname,mortalrate=morrate)
        db.session.add(atte)
        db.session.commit()
...
def edit(id):
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"UPDATE `deaths` SET `ssn`='{ssn}',`name`='{name}',`age`='{age}',`gender`='{gender}',`dname`='{dname}',`email`='{email}',`number`='{num}',`address`='{address}' WHERE `id` = {id}")
...
@app.route('/adddeaths',methods=['POST','GET'])
@login_required
def adddeaths():
    dname=db.engine.execute("SELECT * FROM `diseases`")
    if request.method=="POST":
        ...
        if query1:
            flash("Data Already Exist, Cant add or manipulate","warning")
            return redirect('/adddeaths')
        query=db.engine.execute(f"INSERT INTO `deaths` (`ssn`,`name`,`age`,`gender`,`dname`,`email`,`number`,`address`) VALUES ('{ssn}','{name}','{age}','{gender}','{dname}','{email}','{num}','{address}')")
    

        flash("New Death Added","info")
